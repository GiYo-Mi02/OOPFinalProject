...
export class AdminService {
  private readonly cache = new RedisCache("leaderboard");
  ...
  async getPositionsByElection(electionId: string): Promise<any[]> {
    if (!supabase) throw new Error("Supabase client is not configured.");
  ...
  async createCandidate(payload: {
    ...
    if (position && position.elections) {
      await this.cache.delete((position.elections as any).institute_id);
    }
  ...
  async deleteCandidate(id: string) {
    ...
    if (candidate && candidate.positions) {
      const positions = candidate.positions as any;
  ...
  async getAnalytics() {
    ...
    const instituteBreakdown: Record<string, number> = {};
    institutes?.forEach((vote: any) => {
